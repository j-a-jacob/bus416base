...
To see why pruning matters, we grow a **fully unpenalized tree**. Note that `caret` will not run the full tree as the default is to prune the tree. Thus, the full tree will be run by the `rpart` package and the tuning values will be provided in the `rpart.control()` function.
