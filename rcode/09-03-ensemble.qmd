...
-   Implement bagging, random forests, and boosting using `caret` method
...
#Do a 60-40 split
set.seed(1)
indTrain <- createDataPartition(bank.df$Loan, p = 0.6,  list = F)
train.df <- bank.df[indTrain,]
test.df <- bank.df[-indTrain,]
```

One of the first implementations of this is bagging. Bagging refers to ***B***ootstrapped ***A*****gg**regation
...
3.  Repeat the following steps for k= 1, 2, 3,...K times
    ...
    4.  Update weight as $$ w_{i}^{m+1} = w_{i}^{m} \cdot exp[\alpha _m \cdot I(y_i \ne G_m(x_i))], i = 1, 2, \ldots N  $$
...
In `caret` there are several implementations of `Adaboost`. Here, we will use the method `ada`. Tuning parameters are:

-   `iter` (#Trees)
-   `maxdepth` (Max Tree Depth)
-   `nu` (Learning Rate)

Then rotate with `method = "AdaBoost.M1"` with tuning parameters-
...
Now rotate with `method = "AdaBoost.M1"` with tuning parameters-
...
```{r}
ensemble_results <- bind_rows(ensemble_bag, ensemble_rf, ensemble_ada1, ensemble_adam1 , ensemble_gbm)
