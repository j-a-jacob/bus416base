...
# Exercise:
# Inspect the dataset.
# How many rows and columns does tiny_df contain?
# Identify the predictor variables and the outcome variable.
```
...
#| eval: false
# Exercise:
# Change the number of hidden nodes from 3 to 5.
# Refit the model and inspect the new weights.
...
#| eval: false
# Exercise:
# Extract the predicted probability for the class "like".
# Compare it to the predicted class labels.
...
#| eval: false
# Exercise:
# Manually compute the accuracy from the confusion matrix.
```

### Building a neural network in caret

We can also, very easily build a single-layer neural network in `caret`. Note that while `caret` contains wrapper function for several neural networks, the most commonly used algorithm for classification is the `nnet` package, which _only allows one hidden layer_. __*For more elaborate networks, we will default to MLPClassifier in sklearn.*__
...
#| eval: false
# Exercise:
# Change the size parameter to 5.
# Refit the model and compare the confusion matrix.
```
