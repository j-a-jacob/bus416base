...
Moving foward, we will operate within the `tidyverse` framework. So will use `factor()` or the `forcats()` library for creating factors and `caret` packages, `dummyVars` function to create dummy variables.
...
We begin with a **full model**, including all selected predictors and evaluate in base R.
...
We can use `glance()` to view post-estimation stats like the R-squared
...
In above code chunk, we are using the assignment operator after the pipe operations.
---

## Model performance metrics. A useful function to summarize the performace of the model against the truth is `postRedample()`. 



```{r}
postResample(pred = valid.pred$Pricehat,obs = valid.df$Price)
```

### Interpretation

* Root Mean Squared Error (RMSE) is a good summary measure of how much information is not captured by the model.
