...
The key idea is that: - **Models are interchangeable** - **Evaluation frameworks are not**

Some metrics depend on: - predicted class labels (confusion matrix)

Others depend on: - predicted class probabilities (ROC, AUC, lift)
...
At this stage, **the algorithms themselves are not the focus**. The focus is **how to evaluate them fairly**.
