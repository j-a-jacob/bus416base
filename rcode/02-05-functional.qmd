...
The base `map()` always returns a **list**. We can choose the output using one of the variants.
...
To see the above list as a column vector to make it easy to read:
...
The first `t()` makes the vector into a `1xp` row vector and the next `t()` transposes it to a `px1` column vector.
...
Later on, we will see how a function can be applied within `map()` to create multiple charts looping over selected columns in a dataset.
...
# Compute mean and sd for variables containing "BATH"
