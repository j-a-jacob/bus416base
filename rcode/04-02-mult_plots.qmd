...
The following loop creates **one density plot per variable**, colored by `CHAS`. In the code below, indexing is a bit confusing. In the for loop, we want 3rd to the last cloumn. So, i starts from and ends with the number of columns.
...
One way, which I do not prefer, to do this is using a `for` loop. In this process, we initialize and empty list, create plots one by one, populate the plots in the list and then arrange arrange all of these plots to be viewed on the same page. 
...
### How this works
Recall `02-05-functional` code file. `map()` applies the specified function across all the elements. So, the input on which `map()` will act, is a list of column names. The function that we are applying is to basically insert the data corresponding to that column's name at the x axis of `ggplot`. Where does ggplot look for data? `.data`is a tidyverse pronoun that refers to the data being used in the function. Thus the workflow is:

* `map()` iterates over column names
* `.data[[col_name]]` safely extracts the column by name from the dataframe passed to ggplot (here, df1)
* Each iteration returns a ggplot object
* The result is a **list of plots**. This is because `map()` returns a list.
...
Let's now combine density and histogram. We will add some transparancy to the histogram.
...
plot_list4 <- map(
  ...
  function(col_name) {
    ggplot(df1, aes(x = .data[[col_name]], y = MEDV)) +
      geom_point(aes(color = CHAS), alpha = 0.3) +
