...
We will install a python virtual environment

```{r}
library(reticulate)
virtualenv_create("mlpyenv")
use_virtualenv("mlpyenv", required = TRUE)
py_install(c("numpy", "pandas", "scikit-learn", "matplotlib"), envname = "mlpyenv")
```

Next time you start a new session and want to use this environment, simply type the following code:
